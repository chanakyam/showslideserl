...
welcome(Req, State) ->
	...
	Url_all_news = case Category of 
		"us_political" ->
			%Category = "US",
			string:concat("http://api.contentapi.ws/news?channel=us_politics&limit=20&format=short&skip=", integer_to_list(SkipItems));
		"us_world_news" ->
			%Category = "US",
			string:concat("http://api.contentapi.ws/news?channel=us_world&limit=20&format=short&skip=", integer_to_list(SkipItems));
			
		"us_markets" ->
			%Category = "Politics",
			string:concat("http://api.contentapi.ws/news?channel=us_markets&limit=20&format=short&skip=", integer_to_list(SkipItems));

		"us_technology" ->
			%Category = "Politics",
			string:concat("http://api.contentapi.ws/news?channel=us_technology&limit=20&format=short&skip=", integer_to_list(SkipItems));
		"us_internet" ->
			%Category = "Politics",
			string:concat("http://api.contentapi.ws/news?channel=us_internet&limit=20&format=short&skip=", integer_to_list(SkipItems));
		_ ->
			%Category = "None",
			lager:info("#########################None")
	...
	{ok, Body} = all_news_paginated_dtl:render([{<<"videoParam">>,Params},{<<"news_category">>,Category},{<<"allnews">>,ParamsAllNews}]),
    {Body, Req, State}.
